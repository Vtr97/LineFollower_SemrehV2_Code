...
void controle_com_mapeamento(int encVal)
{
  ...
  if (encVal > 0 && encVal <= 150)
  {
    calcula_PID(Kp, Kd);
    controle_motores(150);
    led_stip.setPixelColor(0, B);
  }

  // primeira curva
  else if (encVal > 150 && encVal <= 760)
  {
    calcula_PID(Kp, Kd);
  ...
  // primeira retinha
  else if (encVal > 760 && encVal <= 1000)
  {
    calcula_PID(Kp, Kd);
  ...
  // trevo
  else if (encVal > 1450 && encVal <= 7500)
  {
    calcula_PID(Kp, Kd);
    controle_motores(170);
    led_stip.setPixelColor(0, B);
  }

  // curva depois do trevo
  else if (encVal > 7500 && encVal <= 8200)
  {
    calcula_PID(Kp, Kd);
    controle_motores(145);
    led_stip.setPixelColor(0, R);
  }

  // antes do U
  else if (encVal > 8200 && encVal <= 8900)
  {
    calcula_PID(Kp, Kd);
    controle_motores(150);
    led_stip.setPixelColor(0, G);
  ...
  // curvinha antes do U
  else if (encVal > 8900 && encVal <= 9300)
  {
    calcula_PID(Kp, Kd);
  ...
  // U
  else if (encVal > 9300 && encVal <= 11000)
  {
    calcula_PID(Kp, Kd);
    controle_motores(130);
    led_stip.setPixelColor(0, R);
  }
...
void ler_laterais(void *parameter)
{
  ...
  while (true)
  {
    ...
    if (medLateralEsq < 3000 || medLateralDir < 3000) // lê marcação esquerdo ou direito
    {
      ...
      {
        SerialBT.print(semicolon);
        SerialBT.print(encoderCountEsq);
        SerialBT.print(semicolon);
        SerialBT.print(encoderCountDir);
        SerialBT.print(semicolon);
        SerialBT.println((encoderCountEsq + encoderCountDir) / 2);
        led_stip.setPixelColor(1, 0, 0, 255);
        led_stip.show();
        digitalWrite(buzzer, HIGH);     // Ligar o buzzer
        vTaskDelay(pdMS_TO_TICKS(100)); // Manter o buzzer ligado por 100ms
